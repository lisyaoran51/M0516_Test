...
void SYS_Init(void)
{
    ...
    CLK_EnableModuleClock(((1UL<<30) |(0x0<<25) |(0x0<<10) |8));

     
    CLK_SetModuleClock(((1UL<<30)|(0x1<<28)|(0x3<<25) |(24<<20)|(0x0<<18)|(0x0F<<10) |( 8<<5)|16), 0x00000000UL, (((1)-1) << 8));
...
int32_t main(void)
{
		...
		uint8_t count;
		char first[16];
		struct CircularLinkedList *inputBuffer, *outputBuffer;
    ...
    printf("Start program.\n");
		...
		inputBuffer = CircularLinkedList_Create();
		outputBuffer = CircularLinkedList_Create();
    ...
    while(1){
			...
			if(ret > 0){
					...
					printf("out size1:%d, size2:%d, size3:%d, size4:%d, buffer size:%d\n", 
					outputBuffer->head->size, 
					outputBuffer->head->next->size,
					outputBuffer->head->next->next->size,
					outputBuffer->head->next->next->next->size,
