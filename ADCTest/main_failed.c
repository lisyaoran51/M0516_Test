...
uint32_t GetAVDDCodeByADC(void)
{
    ...
    //ADC->ADCHER = ((ADC->ADCHER & ~(ADC_ADCHER_CHEN_Msk|ADC_ADCHER_PRESEL_Msk)) | ((1 << 7)|ADC_ADCHER_PRESEL_INT_BANDGAP));
		...
		//ADC->ADCHER |= ((ADC->ADCHER & ~ADC_ADCHER_CHEN_Msk) | (1 << 1));
		//ADC->ADCHER |= ((ADC->ADCHER & ~ADC_ADCHER_CHEN_Msk) | (1 << 2));
		//ADC->ADCHER |= ((ADC->ADCHER & ~ADC_ADCHER_CHEN_Msk) | (1 << 3));
		//ADC->ADCHER |= ((ADC->ADCHER & ~ADC_ADCHER_CHEN_Msk) | (1 << 4));
		//ADC->ADCHER |= ((ADC->ADCHER & ~ADC_ADCHER_CHEN_Msk) | (1 << 5));
		//ADC->ADCHER |= ((ADC->ADCHER & ~ADC_ADCHER_CHEN_Msk) | (1 << 6));
		//ADC->ADCHER |= ((ADC->ADCHER & ~ADC_ADCHER_CHEN_Msk) | (1 << 7));
		//ADC->ADCHER = ((ADC->ADCHER & ~ADC_ADCHER_CHEN_Msk) | 0xFFUL);
    ...
    for(u32Count = 0; u32Count < ADC_SAMPLE_COUNT; u32Count++)
    {
        ...
        /* Get the conversion result */
				...
				for(i = 0; i < 8; i++){
					u32Data = (ADC->ADDR[(i)] & ADC_ADDR_RSLT_Msk) >> ADC_ADDR_RSLT_Pos;
					printf("%d ", u32Data);
				}
				printf("\n");
